...
/**
*Modified GameObject for CSC 165 A2 by Emily Kuang
 * <p>
 * Additions:
 * <ul>
 * <li>Added globalYaw(float) for world-space yaw rotation</li>
 * <li>[Previously in A1] Added yaw(float) and pitch(float)</li>
 * </ul>
 * 
 * <p>Original GameObject implementation by Dr. Scott Gordon</p>
...
public class GameObject
{
	...
	*/
	...
	/**
	 * Applies local-space yaw rotation around y-axis to GameObject
	 * @param angle rotation amount in radians
	 */
	public void yaw (float angle) {
		// rotation matrix around y axis
		Matrix4f rot = new Matrix4f().rotateY(angle);
		localRotation.mul(rot);
		update();
	}
	/**
	 * Applies local-space pitch rotation around x-axis to GameObject
	 * @param angle rotation amount in radians
	 */
	public void pitch (float angle) {
		// rotation matrix around x axis
		Matrix4f rot = new Matrix4f().rotateX(angle);
		localRotation.mul(rot);
		update();
	}

	/**
	 * Rotates given GameObject around global y-axis; rotation is applied
	 * with regard to world-space yaw compared to yaw(float) which does in local
	 * @param angle rotation amount in radians
	 */
	public void globalYaw (float angle) {
		Matrix4f rot = new Matrix4f().rotateY(angle);
		localRotation.mul(rot);
		//this.setLocalRotation(rot.mul(this.getLocalRotation()));
		update();
	}
}