...
/**
 * Modified Camera for CSC 165 A1
 * <p>
 * Additions by Emily Kuang:
 * <ul>
 * <li>Added yaw(float) and pitch(float) in A1</li>
 * </ul>
 * 
 * <p>Original Camera implementation by Dr. Scott Gordon</p>
...
public class Camera
{
	...
	/**
	 * Applies local-space yaw rotation around y-axis to Camera
	 * @param angle rotation amount in radians
	 */
	public void yaw (float angle) {
		// rotation matrix around y axis
		Matrix3f rot = new Matrix3f().rotate(angle, v.x(), v.y(), v.z());
		rot.transform(n);
		rot.transform(u);
		n.normalize();
		u.normalize();
	...
	/**
	 * Applies local-space pitch rotation around x-axis to Camera
	 * @param angle rotation amount in radians
	 */
	public void pitch (float angle) {
		// rotation matrix around x axis
		Matrix3f rot = new Matrix3f().rotate(angle, u.x(), u.y(), u.z());
		rot.transform(n);
		rot.transform(v);
		n.normalize();
		v.normalize();
		u = (new Vector3f(v).cross(n).normalize());
	}
}